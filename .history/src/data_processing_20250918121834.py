...
def preprocess_outbreak_data():
    """Preprocesses NORS data for the outbreak alerts domain with structured metadata."""
    logging.info("Preprocessing Outbreak data...")
    df = pd.read_csv(RAW_DATA_DIR / "outbreaks_data.csv", low_memory=False)

    # Make sure column names are consistent (case-insensitive)
    df.columns = [c.strip() for c in df.columns]

    def build_text(row, idx):
        return (
            f"Outbreak Report ID {idx}. "
    ...
    for index, row in df.iterrows():
        ...
        combined_text = build_text(row, index)

        # Normalize disease and year metadata
        disease = str(row.get('Etiology', '')).strip()
        year = None
        try:
        ...
        # Create chunks and add metadata fields for disease/year
        raw_chunks = create_chunks(combined_text, source_id)
        for c in raw_chunks:
